...
There are multiple data split strategies. For starters, we will split the 30% of the data as a test. This method is the gold standard for testing the performance of our model. By doing this, we have a separate data set that the model has never seen. First, we create a single data frame with predictors and response variables 
```{r mergeLabelTrain}
#For this case, predictor variables are methylation subtypes and response variables are expression values
tgexp=merge(patient,tgexp,by="row.names",all.x=TRUE)
...
Conceptually, k-nearest neighbors (k-NN) is very similar to clustering algorithms we have seen earlier. If we have a measure of the distance between the samples, we can find the nearest $k$ samples to our new sample and use a voting method to decide on the label of our new sample. 
